...
- **주요 이벤트 타입**:
  - 프레임 이벤트: 게임 루프의 시작과 종료를 알리는 이벤트
  - 입력 이벤트: 키보드 및 입력 관련 이벤트
  - 충돌 이벤트: 게임 내 객체 간 충돌을 처리하는 이벤트
  - 상태 변경 이벤트: 게임/개체 상태 변화를 처리하는 이벤트
- **발행-구독 모델**:
  - 이벤트 발행자(Publisher)와 구독자(Subscriber) 간의 느슨한 결합
  - 여러 시스템이 동일한 이벤트에 각각 다른 방식으로 반응 가능
  - 새로운 기능 추가 시 기존 코드 수정 없이 이벤트 구독만으로 확장 가능
- **이벤트 큐 시스템**:
  - 이벤트를 큐에 저장하고 순차적으로 처리하는 메커니즘
  - 동시에 발생하는 다수의 이벤트를 안정적으로 관리
  - 이벤트 우선순위를 통한 처리 순서 제어 지원
